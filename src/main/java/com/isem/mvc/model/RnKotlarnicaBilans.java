...
@Entity
public class RnKotlarnicaBilans {
	@Id
    ...
    @JoinColumn(name = "RN_KOTLARNICA_ID",
            foreignKey = @ForeignKey(name = "RN_KOTLARNICA_BILANS_FK1")
    )
	private RnKotlarnica rnKotlarnica;
    ...
    @JoinColumn(name = "ENERGENT_ID",
            foreignKey = @ForeignKey(name = "RN_KOTLARNICA_BILANS_FK3")
    )
	private Energent energent;
	
	@ManyToOne
    @JoinColumn(name = "DOBAVLJAC_ID",
            foreignKey = @ForeignKey(name = "RN_KOTLARNICA_BILANS_FK4")
    )
	private Dobavljac dobavljac;
	
	@ManyToOne
    @JoinColumn(name = "RN_TIP_ID",
            foreignKey = @ForeignKey(name = "RN_KOTLARNICA_BILANS_FK5")
    )
	private RnTip rnTip;
